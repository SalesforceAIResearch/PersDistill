...
def main():
    ...
    if model_args.model_name_or_path.startswith("codegen-"):
        ...
        if os.path.exists(training_args.output_dir) and os.listdir(training_args.output_dir) and os.path.exists(os.path.join(training_args.output_dir,'config.json')):

            config = AutoConfig.from_pretrained(training_args.output_dir)
            config.gradient_checkpointing = True
            config.use_cache = False
            
            print ("Resuming from checkpoint: ", training_args.output_dir)
            model = AutoModelForCausalLM.from_pretrained(
                training_args.output_dir, config=config, torch_dtype=torch.float16, cache_dir=model_args.cache_dir
            )
        else:
            config = AutoConfig.from_pretrained(f"Salesforce/{model_args.model_name_or_path}-mono")
            config.gradient_checkpointing = True
            config.use_cache = False
            print ("Using the pretrained model ", f"Salesforce/{model_args.model_name_or_path}-mono")
            model = AutoModelForCausalLM.from_pretrained(
                f"Salesforce/{model_args.model_name_or_path}-mono", config=config, torch_dtype=torch.float16, cache_dir=model_args.cache_dir
            )
            print ("Model loaded")
        
        tokenizer = AutoTokenizer.from_pretrained(f"Salesforce/{model_args.model_name_or_path}-mono")
        tokenizer.pad_token = tokenizer.eos_token  #50256
    elif 'starcoder' in model_args.model_name_or_path:
        if os.path.exists(os.path.join(training_args.output_dir, 'config.json')):
            ...
            config = AutoConfig.from_pretrained(f"bigcode/{model_args.model_name_or_path}", use_auth_token=auth_token)
            config.gradient_checkpointing = True
            config.use_cache = False
            print ("Using the pretrained model")
            model = AutoModelForCausalLM.from_pretrained(
                f"bigcode/{model_args.model_name_or_path}", config=config, use_auth_token=auth_token, cache_dir=model_args.cache_dir
            )
        
        tokenizer = AutoTokenizer.from_pretrained(f"bigcode/{model_args.model_name_or_path}", use_auth_token=auth_token)
        tokenizer.pad_token = tokenizer.eos_token
    elif 'models' in model_args.model_name_or_path: # loading prev trained local model
        config = AutoConfig.from_pretrained(model_args.model_name_or_path)
        config.gradient_checkpointing = True
        config.use_cache = False
        model = AutoModelForCausalLM.from_pretrained(
                model_args.model_name_or_path, config=config, torch_dtype=torch.float16, cache_dir=model_args.cache_dir
            )
